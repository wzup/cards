...
module Cards
  class Game < ActiveRecord::Base
    ...
    # Таким образом не принадлежащий этой модели внешний параметр,
    ...
    # Т.е. идти вопреки смыслу паттерна ActiveRecord.
    # 
    ...
    # то модель Games создаст нового юзера.
    # 
    ...
    before_validation() do |game|
      self.user = Cards.user_class.find_or_create_by_id(:id => user_id, :ip => user_ip)
    end
    ...
    # ДЛЯ АЛЬТЕРНАТИВНОГО ВАРИАНТА. Не используется. Сделал лучше.
    # joinGame - присоединиться к игре. Чтобы не засорять контроллер (to keep it as skinny as possible),
    # перенес всю логику данной процедуры в модель.
    # @param game_id string - id данной игры, из params[:id], /en/games/123
    # @param user_id int - id юзера из куки
    # @param remote_ip string - айпишник юзера, "127.0.0.1"
    def self.joinGame(game_id, user_id, remote_ip)
      ...
      self.transaction {
        user = Cards.user_class.find_or_create_by_id(:id => user_id, :ip => user_ip)
    ...
    # @param user_id int - это id юзера из куки
    # @param game_id string | int - это :id игры из урла /games/12345
    # @return #<User> | nil && int - юзер - это @me и роль 1|2|3
    def self.setRole(user_id, game_id)
      user = Cards.user_class.find_by_id(user_id)
    ...
    private
      ...
      # 
      # Чисто domain-specific функция. Нет смысла выносить в плагин или жем.
      # 
